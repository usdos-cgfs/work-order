...
function initVMVars() {
    // set the wo types
    var wotypes = [];
    $.each(woViews, function (key, val) { wotypes.push(val.name) });
...
function closeWorkOrder() {
    var vp = [['RequestStatus', 'Closed'], ['RequestStage', '10'], ['ClosedDate', new Date()]];
    vm.listRefWO().updateListItem(vm.requestHeader().ID, vp, function () { alert('record closed') })
...
function viewWorkOrder(woID) {
    ...
    fetchAttachments();
    fetchComments(function () { console.log('comments fetched') });
...
function buildPipelineElement() {
    ...
    $.each(vm.requestTypeView().pipeline, function (item, stage) {
        ...
        pipeline += '<div class="step ' + status + '">' +
                        '<div class="content">' +
                            '<div class="title">' + stage.type + '</div>' +
                            '<div class="description">' + stage.displayName + '</div>' +
                        '</div></div>';
...
function saveWorkOrder() {
    ...
    */
    SP.UI.ModalDialog.showWaitScreenWithNoClose('Saving Work Order...', 'Please wait...');
    ...
    switch (vm.currentView()) {
        case 'edit':
            ...
            vm.currentListRef().updateListItem(vm.serviceTypeHeader().ID,
                typeValuePairs, onSaveNewWorkOrderMaster)
        ...
        case 'new':
            ...
            vm.currentListRef().createListItem(typeValuePairs, onSaveNewWorkOrderMaster)
            break;
    };
...
function save_tel() {
    ...
    switch (vm.currentView()) {
        ...
        case 'edit':
            // TODO: Pass the correct item 
            vm.currentListRef().updateListItem(vm.serviceTypeHeader().ID, valuePairs, onSaveEditWorkOrderCallback);
            break;
        default:
...
function getValuePairs(listDef) {
    ...
    $.each(listDef, function (field, obj) {
        ...
        if (!(['ID', 'ClosedDate', 'Created'].includes(field))) {
            switch (obj.type){
                case 'DateTime':
                    valuePairs.push([field,
                        !$.isEmptyObject(vm[obj.koMap]()) ? vm[obj.koMap]().toISOString() : ''])
                default:
...
function newAssignment(role) {
    vm.listRefAssignment().showModal('NewForm.aspx', 'New Assignment', { woId: vm.requestID(), role: role }, function (args) {
...
function fetchAllAssignments() {
    var camlq = '<View><Query><Where><Eq><FieldRef Name="Assignee"/><Value Type="Integer"><UserID /></Value></Eq></Where></Query></View>'
    vm.listRefAssignment().getListItems(camlq, function (assignments) {
        console.log(assignments);
        vm.allAssignments(assignments)
        var idarr = [];
        $(vm.allAssignments()).each(function () { idarr.push(this.Title) })
 ...
 ************************************************************/
function newComment() {
    vm.listRefComment().showModal('NewForm.aspx', 'New Comment', { woId: vm.requestID() }, newCommentCallback)
}

function fetchComments(callback) {
    var camlq = '<View><Query><Where><Eq><FieldRef Name="Title"/><Value Type="Text">' + vm.requestID() + '</Value></Eq></Where></Query></View>'
    vm.listRefComment().getListItems(camlq, function (comments) { vm.requestComments(comments); callback() });
}
...
function initApp() {
    ...
    $('.ui.popup').popup();
    $('.menu .item').tab();
    $('.top.menu .item').tab({ 'onVisible': function () { vm.tab(this.id) } });
    ...
    if (href.indexOf('app.aspx') !== -1 || href.indexOf('workorder.aspx') !== -1) {
        vm.page('app');
