...
function initSal() {
    ...
    sal.globalConfig.currentContext.executeQueryAsync(function () {
...
function SPList(listDef) {
    ...
    ******************************************************************/
    self.getFolderContents = function (folderName, callback) {
        ...
        var folder = sal.globalConfig.website.getFolderByServerRelativeUrl(
            sal.globalConfig.siteUrl + '/' + self.config.def.name + '/' + encodeURI(folderName)
            )
    ...
    ******************************************************************/
    ...
    self.showModal = function (formName, title, args, callback) {
        ...
        options.url = sal.globalConfig.siteUrl + '/Lists/' + self.config.def.name + '/' + formName + '?ID=' + id +
    ...
    self.uploadNewDocument = function (folder, title, args, callback) {
        ...
        options.args = JSON.stringify(args);
        options.url = sal.globalConfig.siteUrl + '/_layouts/Upload.aspx?List=' + self.config.guid 
            + '&RootFolder=' + sal.globalConfig.siteUrl + '/' + self.config.def.name + '/' + encodeURI(folder)
            + '&Source=' + location.pathname + '&args=' + encodeURI(JSON.stringify(args));
    ...
    self.createFolder = function (folderName, callback) {
        ...
        var folder = self.config.listRef.get_rootFolder().get_folders().add(folderName);
        self.config.currentContext.load(folder);
        self.config.currentContext.executeQueryAsync(function () {self.requestedCallback()},
            onQueryFailed); 
    }
